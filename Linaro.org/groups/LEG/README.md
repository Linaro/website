...
Leg also leads the initiative [Developer Cloud](https://www.linaro.cloud/). The ARM architecture Developer Cloud allows developers to implement, port and test server, cloud and IoT applications without substantial upfront hardware investment. Developers have access to popular development environments, such as Debian, CentOS, Ubuntu and RHEL. The Developer Cloud (based on OpenStack) employes ARM server platforms from Linaro members AMD, Cavium, HPE, Huawei and Qualcomm. The Developer Cloud is hosted in two regions, one based in Cambridge, UK and the second based in Austin, USA. To sign up for Developer Cloud, click [here](https://register.linaro.cloud/). 
